...
def read_trigger_config(filename):
    ...
    # TODO: Problem 11
    trigger_dict = {}  # Dictionary to store triggers by their names
    trigger_list = []  # List to store triggers to be added to the trigger list

    # Helper function to create trigger objects based on their type and arguments
    def create_trigger(trigger_type, args):
        if trigger_type == "TITLE":
            return TitleTrigger(*args)
        elif trigger_type == "DESCRIPTION":
            return DescriptionTrigger(*args)
        elif trigger_type == "AFTER":
            return AfterTrigger(*args)
        elif trigger_type == "BEFORE":
            return BeforeTrigger(*args)
        elif trigger_type == "NOT":
            return NotTrigger(trigger_dict[args[0]])
        elif trigger_type == "AND":
            return AndTrigger(trigger_dict[args[0]], trigger_dict[args[1]])
        elif trigger_type == "OR":
            return OrTrigger(trigger_dict[args[0]], trigger_dict[args[1]])
    ...
    for line in lines:
        ...
        if action == "ADD":
            ...
            trigger_type = action
            trigger = create_trigger(trigger_type, args)
...
def main_thread(master):
    ...
    try:
        t1 = TitleTrigger("Rain")
        t2 = DescriptionTrigger("pakistan")
        t3 = DescriptionTrigger("England")
        t4 = AndTrigger(t2, t3)
        triggerlist = [t1, t4]
